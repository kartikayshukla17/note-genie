...
import { fetchNotes, clearNotes } from './reducers/noteSlice'
...
const App = () => {
  ...
  const openFiles = openFileIds.map(id => notes.find(n => n.id === id)).filter(Boolean)
  ...
  return (
    ...
    <div className="h-screen w-screen bg-zinc-950 flex text-zinc-300 overflow-hidden font-sans antialiased selection:bg-purple-500/30 selection:text-purple-200">
      ...
      <div className="flex-1 flex flex-col bg-zinc-900/50 m-2 rounded-xl shadow-2xl overflow-hidden border border-white/5 ring-1 ring-black/20 relative z-10 backdrop-blur-sm">
